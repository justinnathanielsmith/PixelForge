      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
            <button 
                ...
                className={`text-[9px] px-2 py-1.5 border fantasy-font font-bold uppercase transition-all rounded-sm shadow-sm flex items-center justify-center gap-1 ${settings.vectorRite ? 'bg-sky-600 border-sky-300 text-black shadow-[0_0_10px_rgba(14,165,233,0.4)]' : 'bg-[#0c0a09] border-stone-700 text-stone-600 hover:text-stone-400'}`}
                title="Toggle Vector-to-Grid Rite (Geometric Silhouette Sharpening)"
            >
                <span className="text-xs">📐</span> Vector Rite
            </button>
            <button 
                onClick={() => updateSetting('temporalStability', !settings.temporalStability)} 
                className={`text-[9px] px-2 py-1.5 border fantasy-font font-bold uppercase transition-all rounded-sm shadow-sm flex items-center justify-center gap-1 ${settings.temporalStability ? 'bg-amber-600 border-amber-300 text-black shadow-[0_0_10px_rgba(217,119,6,0.4)]' : 'bg-[#0c0a09] border-stone-700 text-stone-600 hover:text-stone-400'}`}
                title="Toggle Consistency Engine (Strict Temporal Anchoring)"
            >
                <span className="text-xs">🔄</span> Consistency
            </button>
            <button 
                onClick={() => updateSetting('batchMode', !settings.batchMode)} 
                className={`text-[9px] px-2 py-1.5 border fantasy-font font-bold uppercase transition-all rounded-sm shadow-sm flex items-center justify-center gap-1 ${settings.batchMode ? 'bg-amber-600 border-amber-300 text-black shadow-[0_0_10px_rgba(217,119,6,0.4)]' : 'bg-[#0c0a09] border-stone-700 text-stone-600 hover:text-stone-400'}`}
                title="Toggle Breeding Rite (Generate 4 Variations)"
            >
                <span className="text-xs">🧬</span> Breeding
            </button>
            <button 
                onClick={() => updateSetting('autoTransparency', !settings.autoTransparency)} 
                className={`text-[9px] px-2 py-1.5 border fantasy-font font-bold uppercase transition-all rounded-sm shadow-sm ${settings.autoTransparency ? 'bg-emerald-900 border-emerald-500 text-emerald-100' : 'bg-[#0c0a09] border-stone-700 text-stone-600 hover:text-stone-400'}`}
            ...
            <button 
                ...
                className={`text-[9px] px-2 py-1.5 border fantasy-font font-bold uppercase transition-all rounded-sm shadow-sm ${settings.tiledPreview ? 'bg-amber-900 border-amber-600 text-amber-100' : 'bg-[#0c0a09] border-stone-700 text-stone-600 hover:text-stone-400'}`}
            >
                Seamless
            </button>
            <button 
                onClick={() => updateSetting('showGuides', !settings.showGuides)} 
                className={`text-[9px] px-2 py-1.5 border fantasy-font font-bold uppercase transition-all rounded-sm shadow-sm ${settings.showGuides ? 'bg-sky-900 border-sky-500 text-sky-100' : 'bg-[#0c0a09] border-stone-700 text-stone-600 hover:text-stone-400'}`}
            >
                Guides
            </button>
        </div>

        {/* --- CHROMA TOLERANCE CONTROL --- */}
        {settings.autoTransparency && (
          <div className="space-y-1 bg-[#0c0a09] p-2 border border-emerald-900/30 rounded animate-in slide-in-from-top-1 duration-300">
             <div className="flex justify-between terminal-font text-[9px] text-emerald-500/70 uppercase tracking-tighter">
                <span>Key Tolerance (Edge Cleanup)</span>
                <span>{settings.chromaTolerance}%</span>
             </div>
             <input 
                type="range" 
                min="0" 
        ...
        {settings.batchMode && (
          <div className="bg-amber-950/20 border border-amber-900/40 p-2 rounded text-center space-y-1 animate-in slide-in-from-top-2 duration-300">
             <p className="fantasy-font text-[9px] text-amber-500 uppercase tracking-widest">Breeding Rite Active</p>
             <p className="terminal-font text-[9px] text-stone-500 italic leading-tight">Fixed 2x2 Variation Matrix.</p>
          </div>
        )}
