...
const GalleryModal: React.FC<GalleryModalProps> = ({ isOpen, onClose, history, activeArtId, onSelect, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/95 backdrop-blur-xl p-6 animate-in fade-in duration-300">
      <div className="w-full max-w-6xl h-[85vh] fantasy-card bg-[#0c0a09] flex flex-col border-amber-900/40 shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-[#292524] bg-[#1c1917]">
          <div className="flex items-center gap-4">
             <span className="text-3xl">🖼️</span>
             <div>
               <h2 className="fantasy-font text-xl text-amber-500 uppercase tracking-widest leading-none">The Great Gallery</h2>
          ...
          <div className="flex items-center gap-6">
             {selectedIds.size > 0 && (
               <button 
                 ...
                 className="px-6 py-2 bg-emerald-800 text-emerald-50 border border-emerald-500 fantasy-font text-[10px] font-bold uppercase hover:bg-emerald-700 transition-all rounded shadow-[0_0_15px_rgba(16,185,129,0.2)] animate-in zoom-in-95"
               >
                 Export Selected ({selectedIds.size})
               </button>
             )}
             <button onClick={onClose} className="text-stone-500 hover:text-red-400 text-3xl transition-colors">×</button>
          </div>
        </div>
                ...
                {history.map(art => (
                  ...
                  >
                    ...
                    <div 
                      ...
                      className={`absolute top-2 left-2 w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center ${selectedIds.has(art.id) ? 'bg-amber-600 border-amber-300' : 'bg-black/60 border-stone-700 opacity-0 group-hover:opacity-100'}`}
                    >
        ...
        {/* Footer */}
        <div className="p-4 border-t border-[#292524] bg-[#0c0a09] flex justify-between items-center px-8">
           <p className="text-[10px] terminal-font text-stone-600 uppercase tracking-widest italic">Tip: Use the selection ring for batch artifacts.</p>
           <button onClick={onClose} className="px-10 py-2 bg-stone-900 border border-stone-700 text-stone-500 hover:text-stone-300 fantasy-font text-[10px] uppercase transition-all rounded">Return to Forge</button>
        </div>
      </div>
    </div>
  );
